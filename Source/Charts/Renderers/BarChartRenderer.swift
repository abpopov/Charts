...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        for j in stride(from: 0, to: buffer.rects.count, by: 1)
        {
            ...
            let startPoint = CGPoint(x: barRect.minX, y: barRect.minY)
            let endPoint = CGPoint(x: barRect.maxX, y: barRect.minY)
            
            let gradientBorder:UIBezierPath = UIBezierPath.init(roundedRect: barRect, byRoundingCorners: .allCorners, cornerRadii: CGSize.init(width: 2, height: 2))
            ...
            let contentPath = UIBezierPath.init(roundedRect: barRect.insetBy(dx:2,dy:2), byRoundingCorners: .allCorners, cornerRadii: CGSize.init(width: 2, height: 2))
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        for high in indices
        {
            ...
            if let e = set.entryForXValue(high.x, closestToY: high.y) as? BarChartDataEntry
            {
                ...
                context.setFillColor((dataProvider.barData?.getColors()![0].cgColor)!)
                context.setAlpha(1)
                ...
                let gradientColors = [UIColor(red:  230/255, green: 234/255, blue: 246/255, alpha: 1).cgColor, UIColor(red:  230/255, green: 234/255, blue: 246/255, alpha: 0).cgColor];
                ...
                let startPoint2 = CGPoint(x: barRect.minX, y: barRect.minY)
                let endPoint2 = CGPoint(x: barRect.maxX, y: barRect.minY)
                
                let gradientBorder2:UIBezierPath = UIBezierPath.init(roundedRect: barRect, byRoundingCorners: .allCorners, cornerRadii: CGSize.init(width: 2, height: 2))
                gradientBorder2.addClip()
                ...
                let startPoint = CGPoint(x: barRect.minX, y: barRect.minY)
                let endPoint = CGPoint(x: barRect.minX, y: barRect.maxY)
                
                context.addRect(barRect.insetBy(dx:2,dy:2))
                context.clip()
